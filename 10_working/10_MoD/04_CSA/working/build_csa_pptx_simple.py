"""
Cisco 2025 テンプレートをベースに CSA スライドを生成（別名保存版）
"""
...
output_path = r"c:\Users\kimiito\OneDrive - Cisco\Documents\700 - github\kimiito_Note\10_working\10_MoD\04_CSA\CSA_基礎知識_new.pptx"
...
add_textbox(slide, "Cisco Secure Access", 0.5, 1.8, 12, 1.2,
            font_size=40, bold=True, color=CISCO_ACCENT)
add_textbox(slide, "基礎知識ガイド", 0.5, 2.8, 12, 0.9,
            font_size=30, bold=False, color=CISCO_WHITE)
add_textbox(slide, "SSE 概要 / 実装機能 / Webex 連携 / 設計ベストプラクティス",
            0.5, 3.7, 12, 0.6, font_size=14, color=RGBColor(0xAA, 0xBB, 0xCC))
add_textbox(slide, today, 0.5, 6.5, 4, 0.4, font_size=11, color=TEXT_MUTED)
add_footer(slide)
...
add_card(slide, 0.4,  4.0, 3.9, 2.7, "Better for Users",
         ["• シングルサインオンで即接続", "• クライアント 1 本で全アクセス", "• フリクションレスな UX"], font_size=11)
add_card(slide, 4.5,  4.0, 3.9, 2.7, "Easier for IT",
         ["• 1 コンソール・一元ポリシー管理", "• AI アシスタントでポリシー自動生成", "• 集約レポート"], font_size=11)
add_card(slide, 8.6,  4.0, 4.3, 2.7, "Safer for Everyone",
         ["• 多層防御・最小権限", "• Talos 脅威インテリジェンス", "• 継続的なリスク評価"], font_size=11)
...
positions = [(0.3,  1.0), (4.5,  1.0), (8.7,  1.0), (0.3,  3.8), (4.5,  3.8), (8.7,  3.8)]
for (l, t), (title, body) in zip(positions, cards):
    add_card(slide, l, t, 4.0, 2.6, title, body, font_size=11)
...
add_card(slide, 0.4, 1.7, 5.8, 2.3, "音声に直接影響する機能（優先度：高）",
         ["• FWaaS : ポート/プロトコル許可と制御",
          ...
          "• DEM : 遅延・ジッタ・ロスのリアルタイム可視化"], title_bg=CISCO_BLUE)
...
add_card(slide, 0.4, 1.55, 6.0, 2.6, "Webex Signaling (HTTPS ベース)",
         ["• SWG : 必要ドメイン/URL を許可・危険宛先を遮断",
          ...
          "• DEM : 到達性・応答遅延の可視化",
          "→ 全機能を適用しやすい"], title_bg=CISCO_BLUE)
...
add_card(slide, 0.4, 1.65, 5.8, 3.2, "パッケージ構成",
         ["• Secure Internet Access (SIA) : Web/SaaS 保護",
          ...
          "• DNS Defense 単体パッケージも提供"], title_bg=CISCO_BLUE)
...
add_card(slide, 0.4, 1.0, 6.1, 3.0, "6. Webex で CSA を経由している事例",
         ["確認できたこと（公開資料）:",
          ...
          "→ 部分適用は現実的。全通しは PoC 前提"], title_bg=CISCO_BLUE)
...
add_card(slide, 0.4, 1.6, 5.9, 2.5, "Signaling 推奨設計",
         ["◎ SWG または FWaaS 経由",
          ...
          "• HTTP ヘッダーの改変・削除は非推奨",
          "• プロキシ機能との組み合わせが有効"], title_bg=CISCO_BLUE)
...
add_rect(slide, 0, 0, 13.33, 0.07, CISCO_ACCENT)
add_footer(slide)
add_textbox(slide, "まとめ", 0.5, 0.3, 12, 0.7,
...
col_positions = [(0.4,  1.2, 5.9), (6.5,  1.2, 6.4), (0.4,  4.0, 5.9), (6.5,  4.0, 6.4)]
...
print(f"テンプレートスタイルで 10 枚のスライドを生成しました。")
print(f"（2026 年対応）")
