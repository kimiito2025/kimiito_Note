"""
Cisco 2025 テンプレートをベースに CSA スライドを生成
テンプレートのマスター、配色、フォントを継承して新規スライドを追加
"""
import sys
...
output_path = r"c:\Users\kimiito\OneDrive - Cisco\Documents\700 - github\kimiito_Note\10_working\10_MoD\04_CSA\CSA_基礎知識.pptx"

# Cisco カラースキーム（テンプレート準拠）
...
# テンプレートを開く（またはコピーして新規作成）
if not os.path.exists(template_path):
    print(f"エラー: テンプレートが見つかりません: {template_path}")
    # テンプレートがない場合は、基本的なプレゼンを新規作成
    prs = Presentation()
else:
    # テンプレートを読み込む
    prs = Presentation(template_path)
    # テンプレート内の 2025 を 2026 に置換（テキストレベル）
...
# テンプレートにスライドが既にある場合は、それ以降に追加
# または最後から全て置き換え

# 既存スライドを全て削除して新規構築（テンプレートの場合）
...
print(f"テンプレートベースで 10 枚のスライドを生成しました。")
print(f"（テンプレートに含まれていた 2025 → 2026 に更新）")
