"""
Cisco Secure Access - 基礎知識 PowerPoint 生成スクリプト
Cisco カラースキームで表紙・フッター付き 10 枚スライドを作成する
"""
...
# --- Cisco ブランドカラー -------------------------------------------------
CISCO_BLUE   = RGBColor(0x00, 0x6A, 0xA7)   # 濃い青（タイトルバーなど）
CISCO_DARK   = RGBColor(0x1A, 0x1A, 0x2E)   # 表紙背景
CISCO_ACCENT = RGBColor(0x00, 0xBC, 0xEB)   # ライトシアン（アクセント）
CISCO_WHITE  = RGBColor(0xFF, 0xFF, 0xFF)
CISCO_GRAY   = RGBColor(0xF2, 0xF2, 0xF2)   # スライド背景
TEXT_DARK    = RGBColor(0x1F, 0x2A, 0x36)   # 本文テキスト
TEXT_MUTED   = RGBColor(0x5B, 0x64, 0x72)   # 補足テキスト
CISCO_RED    = RGBColor(0xFF, 0x45, 0x45)   # 注意色
...
def bullet_lines(slide, items, left, top, width, height,
                 font_size=13, color=TEXT_DARK, indent="  "):
    """箇条書きテキストボックス"""
    txb = slide.shapes.add_textbox(
        Inches(left), Inches(top), Inches(width), Inches(height)
    )
...
# -------------------------------------------------------------------------
# Slide 8 - Webex 事例 + SIPS/SRTP 負荷費用
# -------------------------------------------------------------------------
slide = prs.slides.add_slide(blank_layout)
add_rect(slide, 0, 0, 13.33, 7.5, CISCO_GRAY)
...
for (l, t, w), (title, body) in zip(col_positions, summary):
    ...
    for shp in slide.shapes:
        pass  # 色は bullet_lines 内で固定のため個別制御不要


# =========================================================================
# 保存
# =========================================================================
out_path = r"c:\Users\kimiito\OneDrive - Cisco\Documents\700 - github\kimiito_Note\10_working\10_MoD\04_CSA\CSA_基礎知識.pptx"
prs.save(out_path)
